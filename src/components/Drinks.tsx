  ...
  const render_normal = () => {
    return Object.keys(grouped).map((type: string) => {
      ...
      res.push(
        <thead>
          <tr>
            <StyledTData style={{ backgroundColor: 'white', color: 'forestgreen', fontWeight: 'bold', fontSize: 24 }}>{type}</StyledTData>
        ...
        res.push(
          <tbody>
            <tr key={item.id}>
              ...
              <StyledTDataIcons>{!item.noDelivery && <OrderDialog item={item} />}</StyledTDataIcons>
            </tr>
          </tbody>
        );
      });
  ...
  const render_edit = () => {
    return _grouped_mod.map((group: any, index: number) => {
      ...
      res.push(
        <thead>
          <tr>
            <td style={{ backgroundColor: 'dimgrey ', color: 'white', fontSize: '14px', fontWeight: 'bold' }}>
              <TextField
                type="text"
                value={_.isString(group.name) ? group.name : ''}
                onChange={(event) => setGroupMod(_.set(JSON.parse(JSON.stringify(_grouped_mod)), [index, 'name'], event.target.value))}
              />
            </td>
    ...
    >
      <Content>
        ...
        <div style={tableContainerStyle}>
          <StyledTable>{isEditing ? render_edit() : render_normal()}</StyledTable>
        </div>
      </Content>
      <Footer />
    </ImgContainer>
